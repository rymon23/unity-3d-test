...
[System.Serializable]
public struct ZoneConnectorPair
{
...
[ExecuteInEditMode]
public class ProceduralTerrainMesh10 : MonoBehaviour
{
    ...
    [SerializeField] private List<LocationPoint> locationPoints = new List<LocationPoint>();
    ...
    // [SerializeField] private float minLocationZoneCenterYOffset = 0.5f;
    // [SerializeField] private float maxLocationZoneCenterYOffset = 2f;
    ...
    void OnValidate()
    {
        ...
        if (locationPoints == null || locationCount != locationPoints.Count ||
            ...
            )
        {
            ...
            if (locationCount != locationPoints.Count)
            {
        ...
        for (int x = 0; x < terrainSize; x++)
        {
            ...
            for (int y = 0; y < terrainSize; y++)
            {
                ...
                if (locationPoints != null && locationPoints.Count > 0)
                {
                    ...
                    LocationPoint nearestLocation = locationPoints[0];
                    float nearestLocationDist = float.MaxValue;
                    for (int i = 0; i < locationPoints.Count; i++)
                    {
                        float dist = Vector2.Distance(new Vector2(x, y), new Vector2(locationPoints[i].position.x, locationPoints[i].position.z));
                        if (dist < nearestLocationDist)
                        {
                            nearestLocationDist = dist;
                            nearestLocation = locationPoints[i];
                        }
                    }
                    ...
                    // {
                    int subZoneCount = nearestLocation.subZonePoints.Length;
                    ...
                    Vector3 nearestZonePoint = nearestLocation.subZonePoints[0];
                    Vector3 secondNearestZonePoint = nearestLocation.subZonePoints[0];
                    float nearestZoneDist = float.MaxValue;
                    float secondNearestZoneDist = float.MaxValue;
                    for (int i = 0; i < subZoneCount; i++)
                    {
                        float dist = Vector2.Distance(new Vector2(x, y), new Vector2(nearestLocation.subZonePoints[i].x, nearestLocation.subZonePoints[i].z));
                        if (dist < nearestZoneDist)
                        {
                            secondNearestZonePoint = nearestZonePoint;
                            secondNearestZoneDist = nearestZoneDist;
                            nearestZoneDist = dist;
                            nearestZonePoint = nearestLocation.subZonePoints[i];
                        }
                    }
                    ...
                    float centerZoneDistance = Vector2.Distance(new Vector2(x, y), new Vector2(nearestLocation.zonesCenter.x, nearestLocation.zonesCenter.z));
                    List<Vector3> betweenPoints = ProceduralTerrainUtility.GetPointsBetweenPosition(vertices[x + y * terrainSize].position, nearestLocation.subZonePoints);
                    if (betweenPoints.Count > 0)
                    {
                        // if (centerZoneDistance < locationPointZoneCenterRadius)
                        // {
                        //     // float distPercent = 1f - (nearestZoneDist / locationPointZoneRadius);
                        //     float slopeB = Mathf.Lerp(nearestLocation.zonesCenter.y, nearestZonePoint.y, .6f);
                        //     float slope = Mathf.Lerp(lastY, nearestLocation.zonesCenter.y, 1f);

                        //     vertices[x + y * terrainSize].position.y = slope;

                        //     // vertices[x + y * terrainSize].position.y = nearestLocation.zonesCenter.y;
                    ...
                    else
                    {
                        if (centerZoneDistance < locationPointZoneCenterRadius)
                        {
                            float distPercent = 1f - (centerZoneDistance / locationPointZoneCenterRadius);
                            // float slopeB = Mathf.Lerp(nearestLocation.zonesCenter.y, nearestZonePoint.y, .6f);
                            float slope = Mathf.Lerp(lastY, nearestLocation.zonesCenter.y, distPercent);
