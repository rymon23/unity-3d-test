...
public class HeightmapConverterUtil : MonoBehaviour
{
    ...
    [SerializeField] private bool assignPathCells;
    ...
    private void OnValidate()
    {
        if (convertHeightmap)
        {
            ...
            // float[,] zoomedMap = ZoomIn(heights, scale, seed, terrain);
    ...
    float[,] ZoomIn(float[,] heightmap, float scale, int seed, Terrain terrain)
    {
        ...
        int terrain_cols = terrain.terrainData.heightmapResolution;
        float[,] zoomedMap = new float[terrain_rows, terrain_cols];
        int startX = rand.Next(Mathf.Max(rows - (int)(terrain_rows / scale) + 1), rows);
        int startY = rand.Next(Mathf.Max(cols - (int)(terrain_cols / scale) + 1), cols);

        for (int i = 0; i < zoomedMap.GetLength(0); i++)
        {
            for (int j = 0; j < zoomedMap.GetLength(1); j++)
            {
                int x = (int)((i + startX) / scale);
                int y = (int)((j + startY) / scale);
                if (x >= rows || y >= cols)
                {
                    continue; // skip out of bounds values
                }
    ...
    public void DrawCells(HexagonCell[,] cells)
    {
        for (int i = 0; i < cells.GetLength(0); i++)
        {
            for (int j = 0; j < cells.GetLength(1); j++)
            {
                ...
                if (cells[i, j] != null)
                {
                    ...
                    Gizmos.DrawSphere(cellPosition, size);
                }
            }
        }
    }
    ...
    private void OnDrawGizmos()
    {
        if (drawCells)
        {
            if (rectangularHexGrid != null)
            {
                DrawCells(rectangularHexGrid);
            }
        }
    }
