...
public class Faction : ScriptableObject
{
    ...
    GetFactionRelationship(Faction factionA, Faction factionB)
    {
        ...
        Debug
            ...
            factionA.name +
            ": enemies: " +
            factionA.enemyRelations.Count);
        Debug
            .Log("Faction - " +
            factionA.name +
            ": allies: " +
            factionA.allyRelations.Count);

        if (factionA.enemyRelations.Contains(factionB.GetInstanceID()))
            return FactionRelationship.enemy;
        if (factionA.allyRelations.Contains(factionB.GetInstanceID()))
            return FactionRelationship.ally;
        return FactionRelationship.none;
    }

    public static FactionRelationship
    GetMultiFactionRelationship(Faction[] factionsA, Faction[] factionsB)
    {
